...
function getBotResponse(text) {
   botResponse = `<img class="botAvatar" src="./static/img/sara_avatar.jpeg"/>
   <span class="botMsg">${text}</span>
   <div class="clearfix"></div>`;
   /*botResponse = `<img class="botAvatar" src="http://18.223.47.186:8080/ChatBot/static/img/sara_avatar.jpeg"/><span class="botMsg">${text}</span><div class="clearfix"></div>`;*/
  return botResponse;
}
...
function setBotResponse(response) {
  ...
  setTimeout(() => {
    ...
    if (response.length < 1) {
      ...
      const fallbackMsg = "Please enter a valid input";
      
      const BotResponse = `<img class="botAvatar" src="./static/img/sara_avatar.jpeg"/><p class="botMsg">${fallbackMsg}</p><div class="clearfix"></div>`;
     /* const BotResponse = `<img class="botAvatar" src="http://18.223.47.186:8080/ChatBot/static/img/sara_avatar.jpeg"/><p class="botMsg">${fallbackMsg}</p><div class="clearfix"></div>`;*/
      ...
      for (let i = 0; i < response.length; i += 1) {
        ...
        if (Object.hasOwnProperty.call(response[i], "text")) {$("#userInput").prop("disabled", false);
          if (response[i].text != null) {
            ...
            if (
              ...
              if (!botResponse) {
                botResponse = `<img class="botAvatar" src="./static/img/sara_avatar.jpeg"/><p class="botMsg">${response[i].text}</p><div class="clearfix"></div>`;
              }
            }
        ...
        if (Object.hasOwnProperty.call(response[i], "buttons")) {//$("#userInput").prop("disabled", true);
        ...
        if (Object.hasOwnProperty.call(response[i], "custom")) {
          ...
          if (payload === "quickRepliesNew") {
            // check if the custom payload type is "quickReplies"
            const quickRepliesNewData = response[i].custom.data;
            $("#userInput").prop("disabled", false);
            console.log("QuickrepliesNew:",quickRepliesNewData)
            showQuickRepliesNew(quickRepliesNewData);
            
            return;
          }
          
         if (payload === "quickRepliesProduct") {
            // check if the custom payload type is "quickRepliesProduct"
            const quickRepliesProductData = response[i].custom.data;
            //$("#userInput").prop("disabled", true);
            console.log("QuickrepliesProduct:",quickRepliesProductData)
            showQuickRepliesProduct(quickRepliesProductData);
...
function customActionTrigger() {
  $.ajax({
    ...
    //url: "https://34.44.211.46/webhook",
    type: "POST",
    contentType: "application/json",
    data: JSON.stringify({
...
/*function sendAction(action_name) {
  $.ajax({
    url: "http://localhost:5055/webhook/",
    //url: "https://7d98-49-248-76-34.ngrok-free.app/webhook/",
    type: "POST",
    contentType: "application/json",
    data: JSON.stringify({
      next_action: action_name,
...
function customActionTrigger() {
  var action_names = ["action_welcome", "action_welcome1"];

  // Iterate over the array and send one action at a time
  action_names.forEach(function(action_name) {
    sendAction(action_name);
  });
}*/
