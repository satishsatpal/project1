 ...
 * @param {Array} quickRepliesProductData json array
 */

 

function showQuickRepliesProduct(quickRepliesProductData) {
    let chips = "";
    
    for (let i = 0; i < quickRepliesProductData.length; i += 1) {
        const chip = `<div class="chip" data-payload='${quickRepliesProductData[i].payload}'>${quickRepliesProductData[i].title}</div>`;
        
        chips += chip;
    }

    const quickRepliesProduct = `<div class="quickRepliesProduct">${chips}</div><div class="clearfix"></div>`;
    $(quickRepliesProduct).appendTo(".chats").fadeIn();
...
$(document).on("click", ".quickRepliesProduct .chip", function () {
